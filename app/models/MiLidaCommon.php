...
order by creation_time desc";
    ...
    public function getSeriesPackages($search,$stype="all")
    {
        ...
        $sql_packages="SELECT u.firstname foremanfirstname , u.lastname foremanlastname, p.timestmp ptime, sh.*, l.*, p.*, g.*, s.*, pl.* FROM milida.packages p
												LEFT OUTER JOIN groups g on g.group_id=p.group_id
												LEFT OUTER JOIN series s on p.series_id = s.series_id
												LEFT OUTER JOIN preloaded_labels l on p.label_id = l.label_id
												LEFT OUTER JOIN pallets pl on p.pallet_id = pl.pallet_id
    ...
    public function getShiftProductionInfo($gid)
    {
        ...
        WHERE group_id in (select group_id from groups where  shift_id=:shift_id) AND pa.pallet_code is not null GROUP BY p.pallet_id HAVING pcnt < 18";
        ...
        $result['sqltest']="SELECT count(*) pcnt, pa.pallet_code FROM packages_info p left join pallets pa on pa.pallet_id=p.pallet_id
        WHERE group_id in (select group_id from groups where  shift_id=:shift_id) AND pa.pallet_code is not null GROUP BY p.pallet_id HAVING pcnt < ".$result['shift_info']['spallet_capacity'];
        return $result;
    }
