...
*/
    ...
    public function getStorageOverview($wid)  {
          ...
          $sql ="SELECT *,  (select count(*) from packages p where p.series_id=o.series_id and p.location_id =:wid and p.pallet_id is not null) as scnt from overview_by_location_2 o where o.location_id=:wid and o.pallet_code is not null";
        return $this->db->fetchAll($sql, \Phalcon\Db::FETCH_ASSOC, ['wid'=>$wid]);
      }

      public function getShipmentOverview($wid, $stdate, $etdate)  {
        $this->utf8init();
    ...
    public function getShiftSuggestionsInfo($wid){
        ...
        $result['last_serises_data']=$this->db->fetchOne("SELECT series_num, series_year, amount,  (SELECT max(pallet_size) FROM pallets_config where workshop_id=:wid) amountp, weight, product_id, series_id
    ...
    public function getProbeData($serach,$pid,$year,$wid)
    {
       ...
       /* $result['previous']=$this->db->fetchOne("SELECT * FROM probes where seriesId <:seriesId
         ...
         $result['previous'] = $this->db->fetchOne("SELECT * FROM probes where seriesId <> :seriesId order by idprobes desc limit 1",
    ...
    public function updateShipmentItem($data) {
        ...
        $date=date("Y-m-d",intval($data->date_invoice)+60*60*4).' 10:15:00';
    ...
    public function createProbe($data, $uid)
    {
        ...
        if (isset($data->seriesId)&&($data->seriesId>0)) {
            $query ="UPDATE probes SET fat=?, moisture=?, como=?, protein=?, acidity=?, milkAcidity=?,
						 ...
						 expirationTime=?, storingRequirement=?, uid=?, labman=?, labman2=?, labman3=?, standart=?, lactose=?, termoresist =?, timestmp=?, timestmp2=?,  timestmp3=?, rad=? WHERE seriesId = ?";
            ...
            if ($seriesId>0) {
                ...
                `koe`, `yeast`, `bgkp`, `bgkp0`, `expirationTime`, `storingRequirement`, `uid`, `labman`, `labman2`, `labman3`, `standart`,`lactose`, `termoresist`, `timestmp`, `timestmp2`,  `timestmp3`,`rad`)
			VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            array($seriesId,
                $data->fat,
                $data->moisture,
