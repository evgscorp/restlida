...
// http://172.16.130.180/restlida/shift-production/1?token=ufMCdE8EehMSZ7uiQhEVuZfTWbUA8X7yXBxLBufL
	...
	public function getStorageOverviewDates($wid)
	{
		...
		$result = [];
	//if (isset($UserInfo['uid'])&&$UserInfo['uid']>1&&$UserInfo['uid']>0&&in_array(3,$UserInfo['roles'])){
		$result = $MiLidaCommonModel->getStorageOverviewDates($wid);
	 //}
		$Response = $this->allowCORS();
		return $Response->setJsonContent($result);
	}
	...
	public function getUsersList()
	{
		...
		$UserInfo = $MiLidaCommonModel->getUserInfo($this->resource->getAccessToken());
		$result = [];
	if (isset($UserInfo['uid'])&&$UserInfo['uid']>0&&in_array(10,$UserInfo['roles'])){
		$result = $MiLidaCommonModel->getUsersList();
	}
		$Response = $this->allowCORS();
		return $Response->setJsonContent($result);
	}
	...
	public function getAllowedMoves(){
		$request = new \Phalcon\Http\Request();
		$MiLidaCommonModel = new \Models\MiLidaCommon();
		$result = $MiLidaCommonModel->getAllowedMoves($request->get("wid"));
		$Response = $this->allowCORS();
		return $Response->setJsonContent($result);
	}
	...
	public function getProductTypes(){
		$request = new \Phalcon\Http\Request();
		$MiLidaCommonModel = new \Models\MiLidaCommon();
		$result = $MiLidaCommonModel->getProductTypes();
		$Response = $this->allowCORS();
		return $Response->setJsonContent($result);
	}
  ...
  /*
	...
	public function updateUserData(){
		...
		try {
			...
			if (isset($UserInfo['uid'])&&$UserInfo['uid']>0&&in_array(10,$UserInfo['roles'])){
				$data = $this->request->getJsonRawBody();
				$MiLidaCommonModel->updateUserData($data, $UserInfo);
				$res = $data;
			}
		} catch (\Exception $e) {
			$res = 'Error: ' . get_class($e) . ": " . $e->getMessage();
	...
	public function updateSeries($wid){
		try {
			$MiLidaCommonModel = new \Models\MiLidaCommon();
			$UserInfo = $MiLidaCommonModel->getUserInfo($this->resource->getAccessToken());
			if (isset($UserInfo['uid']) && $UserInfo['uid'] > 0) {
				$data = $this->request->getJsonRawBody();
				$MiLidaCommonModel->updateSeries($wid,$data,$UserInfo['uid']);
				$res = $wid;
			}
		} catch (\Exception $e) {
			$res = 'Error: ' . get_class($e) . ": " . $e->getMessage();
